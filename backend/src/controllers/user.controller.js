...
const apiKey = '49263bed8f0342478e723a8fabe41d3b';

const convertAddressToLatLng = async (address, city) => {
  try {
    const url = `https://api.opencagedata.com/geocode/v1/json?q=${encodeURIComponent(address + ', ' + city)}&key=${apiKey}`;
    ...
    console.error('Error converting address to lat/lng:', error.message);
    throw error;
  }
};
...
const updateUserProfile = asyncHandler(async (req, res) => {
  try {
    ...
    console.log(req.body, "req.body");
    const { fullName, email, userType, phone, address, city, gender, description } = req.body;
    const profilePath = req.file?.path;
    const userId = req.user?._id;
    ...
    if (address && city) {
      const location = await convertAddressToLatLng(address, city);
...
// @desc get all cities
// @route GET /api/v1/users/cities
// @access Public
  
// try another logic 

// write a query to get all cities from users collection only cites not other fields
